...
import { OUTLOOK_CALENDAR_CONFIG, validateOutlookCalendarConfig } from '@/config/outlookCalendar';
...
import { OutlookCalendarSettings, CalendarMemberMapping } from '@/types/outlookCalendar';
...
export function OutlookCalendarSync({
  ...
  const [calendars, setCalendars] = useState<Array<{ id: string; name: string; owner?: { name: string; address: string } }>>([]);
  const [settings, setSettings] = useState<OutlookCalendarSettings>({
    enabled: false,
    clientId: OUTLOOK_CALENDAR_CONFIG.CLIENT_ID,
    calendarMappings: [],
    autoSync: false,
    syncInterval: 30,
  });
  ...
  useEffect(() => {
    const savedSettings = StorageService.loadOutlookCalendarSettings();
    if (savedSettings) {
      ...
      if (savedSettings.enabled && validateOutlookCalendarConfig()) {
        initializeOutlookAPI(OUTLOOK_CALENDAR_CONFIG.CLIENT_ID);
      }
    } else if (validateOutlookCalendarConfig()) {
  ...
  const initializeOutlookAPI = async (clientId: string) => {
    try {
      await outlookCalendarService.initialize(clientId);
      setIsAuthenticated(outlookCalendarService.isAuthenticated());
    } catch (error) {
      toast({
        title: 'Initialization Failed',
        description: error instanceof Error ? error.message : 'Failed to initialize Outlook Calendar API',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleAuthenticate = async () => {
    if (!validateOutlookCalendarConfig()) {
      toast({
        title: 'Missing Credentials',
        description: 'Please configure Outlook Calendar credentials in src/config/outlookCalendar.ts',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      await initializeOutlookAPI(OUTLOOK_CALENDAR_CONFIG.CLIENT_ID);
      const authenticated = await outlookCalendarService.authenticate();
      setIsAuthenticated(authenticated);

      if (authenticated) {
        // Fetch calendar list
        const calList = await outlookCalendarService.getCalendarList();
  ...
  const handleSignOut = async () => {
    try {
      ...
      toast({
        title: 'Signed Out',
        description: 'Disconnected from Outlook Calendar',
      });
    } catch (error) {
      toast({
  ...
  const handleUpdateCalendarMapping = (index: number, field: keyof CalendarMemberMapping, value: string) => {
    ...
    if (field === 'calendarId') {
      ...
      updated[index] = {
        ...
        ownerEmail: selectedCal?.owner?.address || '',
      };
    } else {
      updated[index] = {
        ...updated[index],
  ...
  const handleSaveSettings = () => {
    ...
    StorageService.saveOutlookCalendarSettings(settingsToSave);
    toast({
  ...
  // Sync from Outlook Calendar (all mapped calendars) - SAVES TO FIRESTORE
  const handleSyncFromOutlook = async () => {
    ...
    if (!isAuthenticated) {
      toast({
        ...
        description: 'Please connect to Outlook Calendar first',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      // Create EventService for Outlook calendar source
      const outlookEventService = createEventService(familyId, userId, 'outlook');
      ...
      for (const mapping of settings.calendarMappings) {
        try {
          ...
          const outlookEvents = await outlookCalendarService.syncFromOutlook(
            mapping.calendarId,
            mapping.memberId,
            timeMin.toISOString(),
            timeMax.toISOString(),
          ...
          console.log(`✅ Received ${outlookEvents.length} events from Outlook Calendar`);
          
          if (outlookEvents.length > 0) {
            console.log('📝 Sample event:', outlookEvents[0]);
          }

          // Save each event to Firestore in the 'outlook' calendar source
          for (const event of outlookEvents) {
            console.log(`💾 Saving event to Firestore:`, event.title);
            await outlookEventService.createEvent(event);
            totalImported++;
          }
          
          console.log(`✅ Successfully saved ${outlookEvents.length} events to Firestore`);
        } catch (error) {
      ...
      StorageService.saveOutlookCalendarSettings(updatedSettings);
      ...
      toast({
        title: 'Sync Failed',
        description: error instanceof Error ? error.message : 'Failed to sync from Outlook Calendar',
        variant: 'destructive',
      });
    } finally {
      setIsSyncing(false);
  ...
  // Sync to Outlook Calendar - Export local events
  const handleSyncToOutlook = async () => {
    ...
    try {
      ...
      toast({
        title: 'Sync Failed',
        description: error instanceof Error ? error.message : 'Failed to sync to Outlook Calendar',
        variant: 'destructive',
      });
    } finally {
      setIsSyncing(false);
    ...
    <div className="flex items-center gap-2">
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Outlook Calendar Integration</DialogTitle>
            <DialogDescription>
              Connect multiple Outlook/Microsoft Calendars to family members
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor="enabled">Enable Outlook Calendar Sync</Label>
              <Switch
                id="enabled"
                    ...
                    <div className="space-y-2">
                      ...
                      {validateOutlookCalendarConfig() ? (
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-sm">
                            Please update <code className="bg-gray-100 px-1 rounded">src/config/outlookCalendar.ts</code>
                          </p>
                          <p className="text-xs text-gray-600">
                            You'll need to create an Azure App Registration and get a Client ID
                          </p>
                        </div>
                      )}
                    </div>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-start justify-between">
                              <div className="flex-1 space-y-3">
                                {/* Calendar Selection */}
                                <div>
                                  <Label className="text-xs">Outlook Calendar</Label>
                                  <Select
                                      ...
                                      {calendars.map((cal) => (
                                        <SelectItem key={cal.id} value={cal.id}>
                                          <div className="flex flex-col">
                                            <span>{cal.name}</span>
                                            {cal.owner && (
                                              <span className="text-xs text-gray-500">{cal.owner.address}</span>
                                            )}
                                          </div>
                                        </SelectItem>
                                      ))}
